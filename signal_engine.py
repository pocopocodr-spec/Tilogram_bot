...
class GoldSignalEngine:
    ...
    async def get_signal(self):
        ...
        # Поправка под брокерскую цену FXpro GOLDgr (-$0.875/г)
        BROKER_OFFSET = 0.875
        spot_gram = spot_gram - BROKER_OFFSET
        # Пересчитываем oz для коррекции таймфреймов
        spot_oz_adj = spot_gram * TROY_OZ_TO_GRAM

        tf_results = {}
        for tf, df in tf_dfs.items():
            if df is not None:
                try:
                    tf_results[tf] = self._analyze_tf(tf, df, spot_oz_adj)
                except Exception as e:
