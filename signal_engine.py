...
async def get_spot_oz() -> Optional[float]:
    """gold-api.com — бесплатно, без ключа, без лимитов."""
    h = {"User-Agent": "Mozilla/5.0", "Accept": "application/json"}

    # Источник 1: gold-api.com (основной — без ключа, без лимитов)
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get("https://gold-api.com/price/XAU",
                             timeout=aiohttp.ClientTimeout(total=6),
                             headers=h, ssl=False) as r:
                data = await r.json(content_type=None)
                p = float(data.get("price", 0))
                if p > 1000:
                    logger.info(f"Спот (gold-api.com): ${p}/oz")
                    return p
    except Exception as e:
        logger.warning(f"gold-api.com failed: {e}")

    # Источник 2: goldprice.org (резерв)
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get("https://data-asg.goldprice.org/dbXRates/USD",
                ...
                if p and float(p) > 1000:
                    logger.info(f"Спот (goldprice.org): ${p}/oz")
                    return float(p)
    except Exception as e:
        logger.warning(f"goldprice.org failed: {e}")

    # Источник 3: GC=F фьючерс как резерв
    try:
        ...
        if not df.empty:
            ...
            if p > 1000:
                logger.info(f"Спот (GC=F резерв): ${p}/oz")
                return p
    except Exception as e:
...
class GoldSignalEngine:
    ...
    async def get_signal(self):
        ...
        # gold-api.com даёт точную спот-цену — offset не нужен
        spot_oz_adj = spot_oz

        tfs = {}
        for k, df in tf_dfs.items():
            if df is not None:
                try: tfs[k] = self._tf_analyze(k, df, spot_oz_adj)
                except Exception as e:
                    logger.error(f"[{k}] {e}")
                    tfs[k] = None
            else: tfs[k] = None
