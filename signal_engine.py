    ...
    def _fmt(sig: dict, tf_results: dict) -> str:
        ...
        for tf_key in ("D1", "H1", "M15", "M5"):
            ...
            lines += [
                ...
                f"  RSI(7):   <b>{res['rsi']}{rsi_icon}</b>",
                f"  EMA  20:  ${res['e20']:,.2f}  {arr(res['price'], res['e20'])}",
                f"  EMA  50:  ${res['e50']:,.2f}  {arr(res['price'], res['e50'])}",
                f"  EMA 200:  ${res['e200']:,.2f}  {arr(res['price'], res['e200'])}",
                f"  A/D:      {res['ad']}",
                f"  ATR(14):  ${res['atr']}",
                "  <i>Ключевые сигналы:</i>",
            ]
            for s in res["signals"][:3]:   # топ-3, чтобы не раздувать
                lines.append(f"    · {s}")
            lines.append("")
        ...
        if sig["fund_ok"] and sig["fund_heads"]:
            lines.append("  <i>Топ заголовки:</i>")
            for h in sig["fund_heads"]:
                lines.append(f"  · {h}")
        else:
            lines.append("  <i>⚠️ Источники временно недоступны</i>")
        lines.append("")
        ...
        lines += [
            f"⚡ Сила: <b>{sig['strength']}</b>",
            f"   📐 Техн(MTF): {sig['norm_score']:+.1f}  "
            f"📰 Фунд: {sig['fund_score']:+d}  "
            f"Σ {sig['total']:+.1f}",
            "",
            "⚠️ <i>Только информация. Не финансовый совет.</i>",
        ]
        return "\n".join(lines)
