...
class GoldSignalEngine:
    GOLD = "GC=F"   # Gold Futures — цены нативно в USD
    ...
    def _analyze_tf(self, tf_key: str, df: pd.DataFrame) -> dict:
        ...
        p    = float(close.iloc[-1]) / TROY_OZ_TO_GRAM  # цена за 1 грамм
        e20  = self._ema(close, 20) / TROY_OZ_TO_GRAM
        e50  = self._ema(close, 50) / TROY_OZ_TO_GRAM
        e200 = self._ema(close, 200) / TROY_OZ_TO_GRAM
        rsi  = self._rsi(close, 7)
        ad   = self._ad_trend(df)
        atr  = self._atr(df, 14) / TROY_OZ_TO_GRAM
    ...
    def _fmt(sig: dict, tf_results: dict) -> str:
        ...
        ICON = {"BUY": "🟢", "SELL": "🔴", "WAIT": "⚪"}
        DRTX = {"BUY": "ПОКУПКА (BUY)", "SELL": "ПРОДАЖА (SELL)", "WAIT": "ОЖИДАНИЕ (WAIT)"}
        arr  = lambda v, ref: "▲" if v > ref else "▼"

        lines = [
            f"{ICON[drct]} <b>XAUg/USD — {DRTX[drct]}</b>",
            f"🕐 {now}",
            f"💵 Цена за грамм: <b>${p:,.2f}</b>",
            f"🔗 Согласованность TF: <b>{sig['agreement']}%</b>",
            "─────────────────────",
        ]

        # ── Блок по каждому таймфрейму (компактно) ───────────────────────────
        for tf_key in ("D1", "H1", "M15", "M5"):
            res = tf_results.get(tf_key)
            cfg = TIMEFRAMES[tf_key]
            if res is None:
                lines.append(f"{cfg['emoji']} <b>{cfg['label']}</b> ⚠️ нет данных")
                continue

            rsi_icon = (
                "🔥🔥" if res["rsi"] > 80 else "🔥" if res["rsi"] > 65 else
                "💧💧" if res["rsi"] < 20 else "💧" if res["rsi"] < 35 else ""
            )
            lines += [
                f"{cfg['emoji']} <b>{cfg['label']}</b> {res['bias']} (score: {res['score']:+d})",
                f"  RSI:{res['rsi']}{rsi_icon} EMA20:{arr(res['price'],res['e20'])} EMA50:{arr(res['price'],res['e50'])} EMA200:{arr(res['price'],res['e200'])}",
                f"  A/D:{res['ad']}  ATR:${res['atr']}",
            ]
            # только топ-1 сигнал (экранируем HTML)
            if res["signals"]:
                safe_sig = res["signals"][0].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                lines.append(f"  · {safe_sig}")

        # ── Фундаментал ──────────────────────────────────────────────────────
        lines += [
            "─────────────────────",
            f"📰 Фон: <b>{sig['fund_sent']}</b>",
        ]
        if sig["fund_ok"] and sig["fund_heads"]:
            # Экранируем спецсимволы HTML в заголовках
            safe_head = sig['fund_heads'][0].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
            lines.append(f"· {safe_head}")
        ...
        if drct != "WAIT":
            lines += [
                "🎯 <b>Уровни</b>",
                f"Вход: <b>${p:,.2f}</b>  TP1: <b>${sig['tp1']:,.2f}</b>",
                f"TP2: <b>${sig['tp2']:,.2f}</b>  SL: <b>${sig['sl']:,.2f}</b>",
                "─────────────────────",
            ]

        lines += [
            f"⚡ <b>{sig['strength']}</b>  📐{sig['norm_score']:+.1f} 📰{sig['fund_score']:+d} Σ{sig['total']:+.1f}",
