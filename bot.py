...
async def cmd_start(message: Message):
    await message.answer(
        "🥇 <b>Gold Signal Bot — XAUg/USD</b>\n\n"
        "Анализирую золото <b>за грамм в USD</b> каждые <b>15 минут</b>.\n"
...
async def cmd_signal(message: Message):
    wait_msg = await message.answer(
        ...
        "<i>Загружаю котировки и новости (~10 сек)</i>",
        parse_mode="HTML",
    )
    try:
        signal = await engine.get_signal()
        # Убираем все HTML теги из текста для безопасной отправки
        import re
        signal_clean = re.sub(r'<[^>]+>', '', signal)
        await wait_msg.edit_text(signal_clean)
...
async def cmd_status(message: Message):
    ...
    await message.answer(
        ...
        f"  Пара:               XAUg/USD (Золото за грамм / USD)\n"
